...
import { toValue, useDebounceFn, useThrottleFn } from '../../shared';
...
export function useScroll(
  ...
  const scrollTo = (_x: number | undefined, _y: number | undefined) => {
    if (!_window) return;
    ...
    (_element instanceof Document ? _window.document.body : _element).scrollTo({
  ...
  const setArrivedState = (
    ...
    if (target === _window.document && !scrollTop) {
      scrollTop = _window.document.body.scrollTop;
    }
  ...
  const onScrollHandler = (e: Event) => {
    ...
    const eventTarget = (
      e.target === _window.document ? (e.target as Document).documentElement : e.target
    ) as HTMLElement;
